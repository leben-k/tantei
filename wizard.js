...
(function (global) {
  ...
  TanteiWizard.prototype._boot = function () {
    ...
    this.mount.innerHTML =
      ...
      '  <button class="btn btn--sm btn--ghost" id="btnExport" type="button">入力内容をファイルに保存</button>' +
      '  <label class="btn btn--sm btn--ghost" style="cursor:pointer">ファイルから読み込む' +
      ...
      '  <span style="align-self:center;color:var(--ink-soft);font-size:.82rem">この端末に自動保存されます（送信・問い合わせ不要）</span>' +
      '</div>' +
